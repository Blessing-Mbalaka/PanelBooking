...
async function handleSupervisorSearch() {
  const query = supervisorSearch.value.trim();
  if (query.length < 2) {
  ...
  try {
    ...
    if (results.length === 0) {
      supervisorDropdown.innerHTML = "<div style='padding:8px 12px; color:#999; font-size:13px;'>No results found</div>";
      supervisorDropdown.style.display = "block";
      return;
    }

    supervisorDropdown.innerHTML = results.map(function (supervisor) {
      return "<div style='padding:10px 12px; border-bottom:1px solid #f0f0f0; cursor:pointer; font-size:13px; line-height:1.4;' onclick=\"selectSupervisor('" + supervisor.name.replace(/'/g, "\\'") + "')\" onmouseover=\"this.style.background='#f5f5f5';\" onmouseout=\"this.style.background='white';\"><strong>" + supervisor.name + "</strong>" + (supervisor.email ? "<br><span style='color:#999; font-size:12px;'>" + supervisor.email + "</span>" : "") + "</div>";
    }).join("");
    supervisorDropdown.style.display = "block";
  } catch (error) {
    supervisorDropdown.innerHTML = "<div style='padding:8px 12px; color:#c00; font-size:13px;'>Error loading supervisors</div>";
    supervisorDropdown.style.display = "block";
  }
}

function selectSupervisor(name) {
  supervisorSearch.value = name;
...
async function startApp() {
  ...
  supervisorSearch.addEventListener("input", debounce(handleSupervisorSearch, 300));
...
function downloadCSV() {
  ...
  const headers = ["First Name", "Surname", "Email", "Role", "Supervisor", "Date", "Panel", "Slot", "Status", "Booked At"];
  const rows = bookings.map(function (booking) {
    return [
      booking.firstName, booking.surname, booking.email,
      booking.role, booking.supervisorName,
